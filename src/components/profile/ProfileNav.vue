<template>
  <nav class="flex justify-between items-center bg-white rounded-lg gap-8 p-4 md:px-6">
    <!-- LOGO START-->
    <a href="#" class="">
      <img src="/assets/logo-devlinks-small.svg" alt="" class="md:hidden" />
      <img src="/assets/logo-devlinks-large.svg" alt="" class="hidden md:block w-[165px]" />
    </a>
    <!-- LOGO END-->
    ...
    <div class="flex-1 flex items-center justify-center gap-4">
      <button
        ...
        class="border-2 border-transparent rounded-lg text-center font-medium py-3 px-8 flex items-center gap-2 text-lg transition-colors"
        :class="{
          'bg-light-blue text-primary-blue border-light-blue': tab === 'links'
        }"
      >
        <svg
        ...
        >
          <path
            ...
            :class="{
              'fill-primary-blue': tab === 'links',
              'fill-slate-400': tab !== 'links'
            }"
          />
        </svg>
        <span class="hidden md:block whitespace-nowrap">Links</span>
      </button>
      <button
        @click="onSelectTab('details')"
        class="border-2 border-transparent rounded-lg text-center font-medium py-3 px-8 flex items-center gap-2 text-lg transition-colors"
        :class="{
          'bg-light-blue text-primary-blue border-light-blue': tab === 'details'
        }"
      >
        <svg
          xmlns="http://www.w3.org/2000/svg"
        ...
        >
          <path
            :class="{
              'fill-primary-blue': tab === 'details',
              'fill-slate-400': tab !== 'details'
            }"
        ...
        <span class="hidden md:block whitespace-nowrap">Profile Details</span>
      </button>
    </div>
    <!-- Tab Buttons END-->
    ...
    <RouterLink
      :to="{ name: 'preview' }"
      class="border-2 rounded-lg border-primary-blue text-center text-primary-blue font-medium py-3 px-5 md:px-8 hover:bg-primary-blue hover:text-white transition-colors"
    >
      <svg
        xmlns="http://www.w3.org/2000/svg"
        width="20"
